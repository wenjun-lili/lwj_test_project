...
/**
  * @brief   NVIC_Configuration(void)
 *  @datail  SDIO 优先级配置为最高优先级。
 *  @param   none
 *  @return  none
 */
...
/**
  * @brief   SD_DMA_RxConfig(uint32_t *BufferDST, uint32_t BufferSize)
 *  @datail  为SDIO接收数据配置DMA2的通道4的请求
 *  @param   BufferDST：用于装载数据的变量指针
             BufferSize：	缓冲区大小
 *  @return  none
 */
...
/**
  * @brief   SD_DMA_TxConfig(uint32_t *BufferSRC, uint32_t BufferSize)
  * @datail  为SDIO发送数据配置DMA2的通道4的请求
  * @param   BufferDST：装载了数据的变量指针
             BufferSize：	缓冲区大小
  * @return  none
  */
...
/**
  * @brief   GPIO_Configuration(void)
  * @datail  初始化SDIO用到的引脚，开启时钟。
  * @param   none
  * @return  none
  */
...
/**
  * @brief   SD_Init(void)
  * @datail  初始化SD卡，使卡处于就绪状态(准备传输数据)
  * @param   -SD_Error SD卡错误代码   成功时则为 SD_OK
  * @return  none
  */
...
/**
  * @brief  SD_PowerON(void)
  * @datail 确保SD卡的工作电压和配置控制时钟
  * @param  -SD_Error SD卡错误代码  成功时则为 SD_OK
  * @retval SDCardState: SD Card Error or SD Card Current State.
  */
...
/**
  * @brief  SD_PowerOFF(void)
  * @datail 关掉SDIO的输出信号
  * @param  none
  * @retval -SD_Error SD卡错误代码   成功时则为 SD_OK
  */
...
/**
  * @brief  SD_InitializeCards(void)
  * @datail 初始化所有的卡或者单个卡进入就绪状态
  * @param  none
  * @retval -SD_Error SD卡错误代码   成功时则为 SD_OK
  */
...
/**
  * @brief  SD_GetCardInfo(SD_CardInfo *cardinfo)
  * @datail 获取SD卡的具体信息
  * @param  -cardinfo 指向SD_CardInfo结构体的指针
  *         这个结构里面包含了SD卡的具体信息
  * @retval -SD_Error SD卡错误代码   成功时则为 SD_OK
  */
...
/**
  * @brief  SD_EnableWideBusOperation(uint32_t WideMode)
  * @datail 配置卡的数据宽度(但得看卡是否支持)
  * @param -WideMode 指定SD卡的数据线宽
  *         具体可配置如下
  *         @arg SDIO_BusWide_8b: 8-bit data transfer (Only for MMC)
  *         @arg SDIO_BusWide_4b: 4-bit data transfer
  *         @arg SDIO_BusWide_1b: 1-bit data transfer (默认)
  * @return -SD_Error SD卡错误代码
  *         成功时则为 SD_OK
  */
...
/**
  *  @brief  SD_SelectDeselect(uint32_t addr)
  *  @datail 利用cmd7，选择卡相对地址为addr的卡，取消选择其它卡
  *   		   如果addr = 0,则取消选择所有的卡
  *          -addr 选择卡的地址
  *  @return SD_Error SD卡错误代码
  *          成功时则为 SD_OK
