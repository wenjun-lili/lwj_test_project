...
/**
  * @brief  bsp_PutKey(uint8_t _KeyCode)
  * @datail 将1个键值压入按键FIFO缓冲区。可用于模拟一个按键。
  * @param  _KeyCode : 按键代码
  * @return none
  */
...
/**
  * @brief  bsp_GetKey(void)
  * @datail 从按键FIFO缓冲区读取一个键值。
  * @param  none
  * @return none
  */
...
/**
  * @brief  bsp_KeyState(uint8_t _ucKeyID)
  * @datail 读取按键的状态
  * @param  _ucKeyID:按键代码
  * @return none
  */
...
/**
  * @brief  bsp_DetectButton(BUTTON_T *_pBtn)
  * @datail 检测一个按键。非阻塞状态，必须被周期性的调用。
  * @param  _pBtn:按键结构变量指针
  * @return none
  */
static void bsp_DetectButton(BUTTON_T *_pBtn)
{
...
/**
  * @brief  bsp_KeyScan(void)
  * @datail 检测所有按键。非阻塞状态，必须被周期性的调用。
  * @param  none
  * @return none
  */
